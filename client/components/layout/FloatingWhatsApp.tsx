...
export default function FloatingWhatsApp() {
  ...
  const quickReplies = useMemo(
    () => (flow.mode === "booking" ? bookingQuick : baseQuick),
    [flow.mode],
  );
  ...
  useEffect(() => {
    ...
    if (messages.length === 0) {
      setMessages([
        {
          role: "bot",
          text: "Welcome to Nextgen Computing! We repair laptops/desktops, do upgrades, and sell devices & accessories. I’ll guide you step‑by‑step. Choose an option or type your question.",
        },
        {
          role: "bot",
          text: "Popular: Start booking, Services, Shop products, Contact, Location",
        },
      ]);
    }
  ...
  const startBooking = () => {
    ...
    addBot(
      "Step 1/4 — What device do you need help with? (e.g., Laptop, Desktop, Other)",
    );
  };
  ...
  const handleBookingStep = (text: string) => {
    ...
    if (flow.step === 1) {
      ...
      addBot(
        "Step 2/4 — Briefly describe the issue (e.g., won’t boot, screen broken, battery drains, upgrade needed)",
      );
      return;
    }
    ...
    if (flow.step === 2) {
      ...
      addBot(
        "Step 3/4 — Choose service method: Pickup, In‑store, or Remote support",
      );
      return;
    }
    ...
    if (flow.step === 3) {
      ...
      addBot(
        "Step 4/4 — What’s your preferred date/time window? (e.g., tomorrow 3‑5pm)",
      );
      return;
    }
    ...
    if (flow.step === 4) {
      ...
      addBot(
        "Optional — Share your name and phone (e.g., Rahul, 98XXXXXX10). Or type Skip.",
      );
      return;
    }
  ...
  const reply = (text: string) => {
    ...
    if (
      q.includes("start booking") ||
      q.includes("book a repair") ||
      q.includes("book")
    ) {
      startBooking();
      return;
    ...
    if (q.includes("location") || q.includes("address") || q.includes("map")) {
      ...
      addBot(
        "We offer pickup in nearby areas — say ‘Start booking’ to arrange.",
      );
      return;
    }
      ...
      {open && (
        <div className="fixed bottom-24 right-4 w-[calc(100vw-2rem)] sm:w-96 max-h-[70vh] rounded-2xl border border-slate-200/70 bg-white/95 backdrop-blur shadow-2xl flex flex-col overflow-hidden">
          ...
          <div className="px-3 py-2 text-[11px] text-slate-500 bg-slate-50 border-t border-slate-200/70">
            Need a human? Call {PHONE} or email {EMAIL}. For WhatsApp
            confirmation we’ll share a link after booking.
          </div>
        </div>
      )}
    </div>
