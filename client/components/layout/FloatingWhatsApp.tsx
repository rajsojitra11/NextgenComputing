...
export default function FloatingWhatsApp() {
  ...
  type Flow = { mode: "idle" | "booking" | "confirm"; step: number; data: BookingData };
  const [flow, setFlow] = useState<Flow>({ mode: "idle", step: 0, data: {} });

  const baseQuick = ["Start booking", "Services", "Shop products", "Contact", "Location"] as const;
  const bookingQuickByStep: Record<number, string[]> = {
    1: ["Laptop", "Desktop", "Other", "Start over"],
    2: ["Screen broken", "Battery issue", "Won’t boot", "Upgrade", "Start over"],
  ...
  const quickReplies = useMemo(() => {
    ...
    if (flow.mode === "booking") return bookingQuickByStep[flow.step] || ["Start over"];
      ...
      setMessages([
        {
          role: "bot",
          text:
            "Welcome to Nextgen Computing! I’ll guide you step‑by‑step to book a repair, find products, or get our contact/location.",
        },
        { role: "bot", text: "Try: Start booking, Services, Shop products, Contact, Location" },
      ]);
    }
  ...
  const startBooking = () => {
    ...
    addBot("Step 1/5 — What device do you need help with? (e.g., Laptop, Desktop, Other)");
  };
  ...
  const handleBookingStep = (text: string) => {
    ...
    if (flow.step === 1) {
      ...
      addBot("Step 2/5 — Briefly describe the issue (e.g., won’t boot, screen broken, battery drains, upgrade needed)");
      return;
    }
    ...
    if (flow.step === 2) {
      ...
      addBot("Step 3/5 — Choose service method: Pickup, In‑store, or Remote support");
      return;
    }
    ...
    if (flow.step === 3) {
      ...
      addBot("Step 4/5 — What’s your preferred date/time window? (e.g., tomorrow 3‑5pm)");
      return;
    }
    ...
    if (flow.step === 4) {
      ...
      addBot("Step 5/5 — Optional: Share your name and phone (e.g., Rahul, 98XXXXXX10). Or type Skip.");
      return;
    }
    ...
    if (flow.step === 5) {
      ...
      addBot(summary);
      addBot("If this looks good, type ‘Confirm’. You can also ‘Start over’. No WhatsApp link is sent until you Confirm.");
  ...
  const reply = (text: string) => {
    ...
    if (/^services?$|repair|upgrade|data\s*recovery|cleaning|maintenance/i.test(q)) {
      addBot(
    ...
    if (q.includes("contact") || q.includes("phone") || q.includes("call")) {
      addBot(`Phone: ${PHONE}`);
      addBot(`Email: ${EMAIL}`);
      addBot("For instant help, type ‘Start booking’. No WhatsApp link until you confirm.");
      return;
    }
    ...
    if (q.includes("location") || q.includes("address") || q.includes("map") || q.includes("directions")) {
    ...
    if (q.includes("warranty") || q.includes("guarantee") || q.includes("price") || q.includes("pricing") || q.includes("cost")) {
      addBot(
    ...
    addBot("I can help with Start booking, Services, Products, Contact, and Location. Try the quick buttons below.");
  };
      ...
      {open && (
        <div className="fixed bottom-24 right-4 w-[calc(100vw-2rem)] sm:w-96 max-h-[70vh] rounded-2xl border border-slate-200/70 bg-white/95 backdrop-blur shadow-2xl flex flex-col overflow-hidden">
          ...
          <div className="px-3 py-2 text-[11px] text-slate-500 bg-slate-50 border-t border-slate-200/70">
            Need a human? Call {PHONE} or email {EMAIL}. WhatsApp link is only shown after you confirm.
          </div>
        </div>
      )}
    </div>
